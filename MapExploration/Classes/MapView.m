...
- (void) createPinsFromDB {
	...
	NSString* sqlstatement = @"select courtname, address, latitude, longitude, courts, city, rating from tenniscourts";
	...
	while ((dbrc = sqlite3_step(sqlite_stmt)) == SQLITE_ROW) {
		...
		PinAnnotation *pinAnnotation =[[[PinAnnotation alloc] initWithCoordinate:coordinate name:name address:address city:city numCourts:courts rating: rating] autorelease];
		[name release];
		[address release];
		[city release];
