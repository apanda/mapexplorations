...
for row in c:
    if str(row[1]) != '' and str(row[2]) != '':
        ...
        try:
            ...
            c2.execute("insert into tenniscourts (courtname, address, city, neighborhood, courts, latitude, longitude, rating) values (?, ?, ?, ?, ?, ?, ?, ?)", (row[0], row[1], row[2], row[3], row[4], lat, lng, 0))
            print out
        except ValueError:
            print "valerror"
